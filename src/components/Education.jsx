...
export default function Education() {
  ...
  return (
    <section id="education" ref={ref} style={{ padding: "clamp(80px,10vw,140px) clamp(20px,4vw,40px)", background: "#f5f5f0", position: "relative", overflow: "hidden" }}>
      <div
        style={{
          position: "absolute", top: "50%", left: "50%",
          transform: `translate(-50%, calc(-50% + ${off}px))`,
          fontSize: "clamp(60px,18vw,260px)", fontWeight: 900, color: "rgba(0,0,0,0.025)",
      ...
      <div style={{ maxWidth: 1100, margin: "0 auto", position: "relative" }}>
        ...
        {EDUCATION.map((edu, i) => (
          <Reveal key={i} dir={i % 2 === 0 ? "left" : "right"} delay={0.12}>
            ...
            >
              {edu.current && (
                <span style={{ position: "absolute", top: 36, right: 0, fontSize: 10, fontWeight: 700, letterSpacing: 2, textTransform: "uppercase", color: "#fff", background: "#111", padding: "4px 12px", borderRadius: 999 }}>Current</span>
              )}
              <div style={{ fontSize: 12, color: "#999", letterSpacing: 2, fontWeight: 500, marginBottom: 12 }}>{edu.period}</div>
              <h3 style={{ fontSize: "clamp(20px,3vw,36px)", fontWeight: 800, margin: "0 0 6px", letterSpacing: -0.5 }}>{edu.degree}</h3>
              <div style={{ fontSize: "clamp(14px,1.2vw,17px)", color: "#555", fontWeight: 600, marginBottom: 4 }}>{edu.school}</div>
      ...
      <style>{`
        @media (max-width: 768px) {
          .edu-block { margin-left: 0 !important; margin-right: 0 !important; }
        }
      `}</style>
    </section>
  );
